...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\SATO\\OneDrive\\Coding\\Dr.Sato\\exp_202311\\4-3_ResumeGaze-Beep_lastrun.py',
...
# --- Initialize components for Routine "Exer_Eyecatch_2" ---
...
time.sleep(1)
# Run 'Begin Routine' code from p_init
...
exer_trials = data.TrialHandler(nReps=2.0, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('xlsx/ResumeGazewithBeep.xlsx'),
...
for thisExer_trial in exer_trials:
    ...
    _key_resp_7_allKeys = []
    # keep track of which components have finished
    CheckComponents = [check_l_msg, led, key_resp_7]
    ...
    exer_trials.addData('key_resp_7.keys',key_resp_7.keys)
    if key_resp_7.keys != None:  # we had a response
        exer_trials.addData('key_resp_7.rt', key_resp_7.rt)
...
# completed 2.0 repeats of 'exer_trials'
...
# --- Prepare to start Routine "Exer_Eyecatch_2" ---
continueRoutine = True
routineForceEnded = False
# update component parameters for each repeat
key_resp_4.keys = []
...
_key_resp_4_allKeys = []
# keep track of which components have finished
Exer_Eyecatch_2Components = [exer_msg_2, key_resp_4]
for thisComponent in Exer_Eyecatch_2Components:
    thisComponent.tStart = None
    thisComponent.tStop = None
    thisComponent.tStartRefresh = None
    thisComponent.tStopRefresh = None
...
while continueRoutine:
    ...
    # *exer_msg_2* updates
    if exer_msg_2.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
...
# --- Ending Routine "Exer_Eyecatch_2" ---
for thisComponent in Exer_Eyecatch_2Components:
    if hasattr(thisComponent, "setAutoDraw"):
        thisComponent.setAutoDraw(False)
# the Routine "Exer_Eyecatch_2" was not non-slip safe, so reset the non-slip timer
routineTimer.reset()
...
# set up handler to look after randomisation of conditions etc
exer_trials_2 = data.TrialHandler(nReps=2.0, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('xlsx/ResumeGazewithBeep.xlsx'),
    seed=None, name='exer_trials_2')
thisExp.addLoop(exer_trials_2)  # add the loop to the experiment
thisExer_trial_2 = exer_trials_2.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisExer_trial_2.rgb)
if thisExer_trial_2 != None:
    for paramName in thisExer_trial_2:
        exec('{} = thisExer_trial_2[paramName]'.format(paramName))

for thisExer_trial_2 in exer_trials_2:
    currentLoop = exer_trials_2
    # abbreviate parameter names if possible (e.g. rgb = thisExer_trial_2.rgb)
    if thisExer_trial_2 != None:
        for paramName in thisExer_trial_2:
            exec('{} = thisExer_trial_2[paramName]'.format(paramName))
    ...
    if led.keys in ['', [], None]:  # No response was made
        led.keys = None
    exer_trials_2.addData('led.keys',led.keys)
    if led.keys != None:  # we had a response
        exer_trials_2.addData('led.rt', led.rt)
    # check responses
    if key_resp_7.keys in ['', [], None]:  # No response was made
        key_resp_7.keys = None
    exer_trials_2.addData('key_resp_7.keys',key_resp_7.keys)
    if key_resp_7.keys != None:  # we had a response
        exer_trials_2.addData('key_resp_7.rt', key_resp_7.rt)
    # the Routine "Check" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
...
# completed 2.0 repeats of 'exer_trials_2'
...
# set up handler to look after randomisation of conditions etc
test_trials = data.TrialHandler(nReps=24.0, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('xlsx/ResumeGazewithBeep.xlsx'),
...
for thisTest_trial in test_trials:
    ...
    if key_resp_7.keys in ['', [], None]:  # No response was made
        key_resp_7.keys = None
    test_trials.addData('key_resp_7.keys',key_resp_7.keys)
    if key_resp_7.keys != None:  # we had a response
        test_trials.addData('key_resp_7.rt', key_resp_7.rt)
    # the Routine "Check" was not non-slip safe, so reset the non-slip timer
    routineTimer.reset()
...
# completed 24.0 repeats of 'test_trials'
