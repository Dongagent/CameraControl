...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='C:\\Users\\smart\\OneDrive\\Coding\\Dr.Sato\\202310_PsychoPy\\untitled_lastrun.py',
...
inputnum = visual.TextStim(win=win, name='inputnum',
    text=None,
    font='Open Sans',
    pos=(0, 0), height=0.05, wrapWidth=None, ori=0.0, 
    color='white', colorSpace='rgb', opacity=None, 
...
# keep track of which components have finished
trialComponents = [text_4, key, mouse, key_resp, inputnum]
...
while continueRoutine:
    ...
    # *key* updates
    if key.status == NOT_STARTED and t >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        ...
        key.clock.reset()  # now t=0
        key.clearEvents(eventType='keyboard')
    if key.status == STARTED:
        theseKeys = key.getKeys(keyList=['1', '2','3','4','5','6','7','8','9','q'], waitRelease=False)
    ...
    if keys:
        ...
        if displayText=='1':
            ...
            text_ = "ミスです。1~9の数字を入力してください。"
            #flag=0
        inputnum.setText(displayText)
    
    # *inputnum* updates
    if inputnum.status == NOT_STARTED and tThisFlip >= 0.0-frameTolerance:
        # keep track of start time/frame for later
        inputnum.frameNStart = frameN  # exact frame index
        inputnum.tStart = t  # local t and not account for scr refresh
        inputnum.tStartRefresh = tThisFlipGlobal  # on global time
        win.timeOnFlip(inputnum, 'tStartRefresh')  # time at next scr refresh
        inputnum.setAutoDraw(True)
